# Supercharger Docs

Documentation site for [Supercharger](https://github.com/mrpolanco/Supercharger) — an open-source learning platform where your AI coding assistant is the curriculum engine.

**Live site:** https://mrpolanco.github.io/Supercharger-docs/

Built with [Astro Starlight](https://starlight.astro.build). Deployed to GitHub Pages via `.github/workflows/deploy.yml`.

```bash
npm install
npm run dev      # local preview
npm run build    # production build to dist/
```
